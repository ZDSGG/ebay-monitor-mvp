...
@router.post("/run-daily-shop-scan", response_model=CrawlTriggerResponse)
def run_daily_shop_scan(
    x_cron_secret: str | None = Header(default=None),
    db: Session = Depends(get_db),
) -> CrawlTriggerResponse:
    if not settings.cron_secret:
    ...
    jobs = ShopScanService(db).scan_active_shops(trigger_source="CRON")
    return CrawlTriggerResponse(
        requested=len(jobs),
        succeeded=sum(1 for job in jobs if job.status == JobStatus.SUCCESS),
        failed=sum(1 for job in jobs if job.status == JobStatus.FAILED),
